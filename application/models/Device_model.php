...
class Device_model extends CI_Model{
    ...
    function insert_m_file_iec($id_device){
        ...
        (domain_id,item_id,active,id_device,alias,name,data_type)
        values ("IA","IA",1,'.$id_device.',(SELECT CONCAT(kode_mesin,"'.$id_device.'IA") FROM m_mesin ),"Current A","float"),
               ("IB","IB",1,'.$id_device.',(SELECT CONCAT(kode_mesin,"'.$id_device.'IB") FROM m_mesin ),"Current B","float"),
               ("IC","IC",1,'.$id_device.',(SELECT CONCAT(kode_mesin,"'.$id_device.'IC") FROM m_mesin ),"Current C","float"),
               ("IN","IN",1,'.$id_device.',(SELECT CONCAT(kode_mesin,"'.$id_device.'IN") FROM m_mesin ),"Current N","float"),
               ("alarm","alarm",1,'.$id_device.',(SELECT CONCAT(kode_mesin,"'.$id_device.'ALARM") FROM m_mesin ),"alarm","boolean"),
               ("trip","trip",1,'.$id_device.',(SELECT CONCAT(kode_mesin,"'.$id_device.'TRIP") FROM m_mesin ),"trip","boolean"),
               ("t1","t1",1,'.$id_device.',(SELECT CONCAT(kode_mesin,"'.$id_device.'T1") FROM m_mesin ),"t1","boolean"),
               ("t2","t2",1,'.$id_device.',(SELECT CONCAT(kode_mesin,"'.$id_device.'T2") FROM m_mesin ),"t2","boolean"),
               ("t3","t3",1,'.$id_device.',(SELECT CONCAT(kode_mesin,"'.$id_device.'T3") FROM m_mesin ),"t3","boolean"),
               ("t4","t4",1,'.$id_device.',(SELECT CONCAT(kode_mesin,"'.$id_device.'T4") FROM m_mesin ),"t4","boolean"); ';
