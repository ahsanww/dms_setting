...
class Scl_model extends CI_Model{
    ...
    function get_scl_bydevice($id){
      $qry = "SELECT a.*,b.port_type from m_file_iec a left join device_list b on a.id_device = b.id_device WHERE a.id_device = '$id'";
    ...
    function get_scl_byid($id){
      $this->db->where('m_file_iec.id',$id);
      $this->db->from("m_file_iec");
      $this->db->join("device_list",'m_file_iec.id_device = device_list.id_device');
