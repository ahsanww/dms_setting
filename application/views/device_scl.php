...
?> 
  ...
  <div class="content-wrapper">
    ...
    <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1 class="m-0 text-dark">Generate CID Files</h1>
          </div><!-- /.col -->
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              <li class="breadcrumb-item"><a href="../admin/dashboard">Home</a></li>
              <li class="breadcrumb-item active">Generate CID Files</li>
            </ol>
          </div><!-- /.col -->
        </div><!-- /.row -->
      </div><!-- /.container-fluid -->
              ...
              <div class="card-header p-0 pt-1">
                ...
                <li class="nav-item">
                  ...
                  <li class="nav-item iec" >
                    <a class="nav-link active" id="tab-SCL" data-toggle="pill" href="#custom-tabs-one-profile" role="tab" aria-controls="custom-tabs-one-profile" aria-selected="true">Generate CID files &nbsp <i class=" text-success fa fa-search"></i></a>
                  </li>
                  <li class="nav-item">
                    <a class="nav-link" id="list" data-toggle="pill" href="list" onClick="toactive(<?php echo $id?>,<?php echo $no?>)" role="tab" aria-controls="custom-tabs-one-messages" aria-selected="false">CID Activation &nbsp <i class=" text-white fa fa-check"></i></a>
                  </li>
                </ul>
              </div>
              <div class="card-body">
                    ...
                    <div class="">  
                      ...
                      <form action="do_upload" id="form_upload" method="POST" enctype="multipart/form-data">
                       <?php //echo form_open_multipart('scl/do_upload');?>
                        ...
                        <label for="userfile"> Upload CID FIles</label>
                        <input  id="file" type="file" name="userfile" size="300" required />
  ...
  <script>
    ...
    function getData(x)
    {
      $.get(`../../assets/scl/${x}.json`,(data,status)=>{
        console.log(data)
  
          buf= (data)
          for(let i =0 ;  i<buf.length;i++){
            buf2=buf[i].all.split('$')
            //console.log(buf2)
            b=""
            c=""
            a=""
    ...
    function getStatusScl(){
      ...
      $.get(`get_status_scl?no=${no}`,(res,status)=>{
          ...
          if(kondisi ==1){
            ...
            console.log("gagal baca dul")
            $('#msg').empty()
            $('#msg').append(`<span class="text-danger text-weight">Failed to Load SCL<s/pan> `)
             
            //$('#msg').empty()
            //$('#msg').append(`<div class="s text-danger" role="status"><span class="sr-only">Failed Load SCL`)
